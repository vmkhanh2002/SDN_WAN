...
def initialize_agents() -> None:
    ...
    if _crewai_available:
        for task_name, instruction in _KNOWN_TASKS:
            try:
                a = Agent(name=task_name)
                # Attach a short instruction task (not payload-bearing)
                try:
                    a.add_task(Task(name=f"{task_name}-instruction", instruction=instruction))
                except Exception:
                    # Some Agent implementations may not support add_task; ignore
                    logging.debug("Agent.add_task not supported for %s", task_name)
                _agents[task_name] = {"type": "crew", "agent": a}
            except Exception:
                logging.exception("Failed to construct crew Agent for %s, falling back to stub", task_name)
